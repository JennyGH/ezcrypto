...
public:
    ...
    void init_key(const byte_t* key, size_t key_length)
    {
        uint8_t actual_key[56] = {0};
        key_length             = std::min<size_t>(sizeof(key), key_length);
        ::memcpy(actual_key, key, key_length);

        const uint8_t* p = actual_key;
        ...
        for (size_t j = 0; j < 4; j++)
        {
            ...
            {
                encrypt(block);
                _key_sbox[j][k++] = block[0];
                _key_sbox[j][k++] = block[1];
            }
        }
    }

    void encrypt(uint32_t (&x)[2])
    {
        uint32_t xl = x[0];
        uint32_t xr = x[1];
        xl ^= _key_pbox[0];
        ...
        xl ^= F(xr) ^ _key_pbox[8];
        xr ^= F(xl) ^ _key_pbox[9];
        xl ^= F(xr) ^ _key_pbox[10];
        xr ^= F(xl) ^ _key_pbox[11];
        xl ^= F(xr) ^ _key_pbox[12];
        xr ^= F(xl) ^ _key_pbox[13];
        xl ^= F(xr) ^ _key_pbox[14];
        xr ^= F(xl) ^ _key_pbox[15];
        xl ^= F(xr) ^ _key_pbox[16];
        xr ^= _key_pbox[17];
        x[0] = xr;
        x[1] = xl;
    }

    void decrypt(uint32_t (&x)[2])
    {
        uint32_t xl = x[0];
        uint32_t xr = x[1];
        xl ^= _key_pbox[17];
        xr ^= F(xl) ^ _key_pbox[16];
        xl ^= F(xr) ^ _key_pbox[15];
        xr ^= F(xl) ^ _key_pbox[14];
        xl ^= F(xr) ^ _key_pbox[13];
        xr ^= F(xl) ^ _key_pbox[12];
        xl ^= F(xr) ^ _key_pbox[11];
        xr ^= F(xl) ^ _key_pbox[10];
        xl ^= F(xr) ^ _key_pbox[9];
        xr ^= F(xl) ^ _key_pbox[8];
        xl ^= F(xr) ^ _key_pbox[7];
        xr ^= F(xl) ^ _key_pbox[6];
